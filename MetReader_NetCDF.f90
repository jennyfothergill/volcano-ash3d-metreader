...
!##############################################################################
      ...
      write(*,*)"--------------------------------------------------------------------------------"
      write(*,*)"----------                MR_Read_Met_DimVars_netcdf                  ----------"
      write(*,*)"--------------------------------------------------------------------------------"
      ...
      ELSEIF(MR_iwindformat.eq.3)THEN
        ...
        Met_var_names(1) = "Geopotential_height_isobaric"       ; Met_var_IsAvailable(1)=.true.
        Met_var_names(2) = "u_wind_isobaric"                    ; Met_var_IsAvailable(2)=.true.
        Met_var_names(3) = "v_wind_isobaric"                    ; Met_var_IsAvailable(3)=.true.
        Met_var_names(4) = "Pressure_vertical_velocity_isobaric"; Met_var_IsAvailable(4)=.true.
        Met_var_names(5) = "Temp_isobaric"               ; Met_var_IsAvailable(5)=.true.
        ! Moisture
        Met_var_names(31) = "Specific_humidity_isobaric"        ; Met_var_IsAvailable(31)=.true.
        Met_var_names(32) = "Cloud_water_isobaric"              ; Met_var_IsAvailable(32)=.true.
      ...
      ELSEIF(MR_iwindformat.eq.4)THEN
        ...
        Met_var_names(44) = "Precipitation_rate"
      ...
      ELSEIF(MR_iwindformat.eq.9)THEN
        ...
        Met_dim_names(1) = "time"      ! time
          Met_dim_IsAvailable(1)=.true.
        Met_dim_names(2) = "pressure1" ! pressure
          Met_dim_IsAvailable(2)=.true.
        Met_dim_names(3) = "y"         ! y
          Met_dim_IsAvailable(3)=.true.
        Met_dim_names(4) = "x"         ! x
          Met_dim_IsAvailable(4)=.true.
        Met_dim_names(5) = "pressure1" ! pressure coordinate for Vz
          Met_dim_IsAvailable(5)=.true.
        Met_dim_names(6) = "pressure1" ! pressure coordinate for RH
          Met_dim_IsAvailable(6)=.true.

        ! Mechanical / State variables
        Met_var_names(1) = "Geopotential_height"    !        float 
          Met_var_IsAvailable(1)=.true.
        Met_var_names(2) = "U-component_of_wind"    !        float 
          Met_var_IsAvailable(2)=.true.
        Met_var_names(3) = "V-component_of_wind"    !        float 
          Met_var_IsAvailable(3)=.true.
        Met_var_names(4) = "Vertical_velocity_pressure"  ! float
          Met_var_IsAvailable(4)=.true.
        Met_var_names(5) = "Temperature"            !        float 
          Met_var_IsAvailable(5)=.true.
        ! Moisture
        Met_var_names(30) = "Relative_humidity"      !        float 
          Met_var_IsAvailable(30)=.true.
      ...
      ELSEIF (MR_iwindformat.eq.21) THEN
       ...
       ELSEIF (MR_iwindformat.eq.23) THEN
        ...
        Met_var_names(30) = "Relative_humidity"         ! float percent
          Met_var_IsAvailable(30)=.true.
        ! Precipitation
        Met_var_names(44) = "Precipitation_rate"         ! float kg/m2/s (all 0's)
          Met_var_IsAvailable(44)=.true.
       ...
       ELSEIF (MR_iwindformat.eq.32) THEN
        ...
        Met_dim_names(1) = "time"       ! time
          Met_dim_IsAvailable(1)=.true. 
        Met_dim_names(2) = "isobaric3"      ! pressure (39 levels 1013 -> 0.05)
          Met_dim_IsAvailable(2)=.true. 
        Met_dim_names(3) = "lat"      ! y        (34.5 -> -40.32)
          Met_dim_IsAvailable(3)=.true. 
        Met_dim_names(4) = "lon"      ! x        (12.5 -> 18.0)
...
!##############################################################################
      ...
      write(*,*)"About to open first WRF file : ",MR_windfiles(1)
      nSTAT=nf90_open(adjustl(trim(MR_windfiles(1))),NF90_NOWRITE, ncid)
      IF(nSTAT.ne.0) THEN
        write(9,*)'MR ERROR: open WRF file: ',nf90_strerror(nSTAT)
        write(6,*)'MR ERROR: open WRF file: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_dimid Time: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_dimid Time: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF
      nSTAT = nf90_Inquire_Dimension(ncid,t_dim_id,name=name_dum,len=nt_fullmet)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: Inquire_Dimension Time: ', &
                             nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: Inquire_Dimension Time: ', &
                             nf90_strerror(nSTAT)
        stop 1
      ENDIF 
      nSTAT = nf90_inq_dimid(ncid,Met_dim_names(4),x_dim_id)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_dimid x: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_dimid x: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF

      nSTAT = nf90_Inquire_Dimension(ncid,x_dim_id,name=name_dum,len=nx_fullmet)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: Inquire_Dimension x: ', &
                             nf90_strerror(nSTAT)
        write(6,*)'MR ERROR: Inquire_Dimension x: ', &
                             nf90_strerror(nSTAT)
        stop 1
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_dimid y: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_dimid y: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF

      nSTAT = nf90_Inquire_Dimension(ncid,y_dim_id,name=name_dum,len=ny_fullmet)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: Inquire_Dimension y: ', &   
                             nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: Inquire_Dimension y: ', &
                             nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_dimid z: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_dimid z: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF

      nSTAT = nf90_Inquire_Dimension(ncid,z_dim_id,name=name_dum,len=neta_fullmet)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: Inquire_Dimension z: ', &
                             nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: Inquire_Dimension z: ', &
                             nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: get_att MAP_PROJ: ', &
                             nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: get_att MAP_PROJ: ', &
                             nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      if(Map_Proj.eq.1)THEN
        ...
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_att DX: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_att DX: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        nSTAT = nf90_get_att(ncid, NF90_GLOBAL, "DY", WRF_dy)
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_att DY: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_att DY: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        nSTAT = nf90_get_att(ncid, NF90_GLOBAL, "CEN_LAT", Cen_Lat)
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_att CEN_LAT: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_att CEN_LAT: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        nSTAT = nf90_get_att(ncid, NF90_GLOBAL, "STAND_LON", Stand_Lon)
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_att STAND_LON: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_att STAND_LON: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        nSTAT = nf90_get_att(ncid, NF90_GLOBAL, "TRUELAT1", Truelat1)
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_att TRUELAT1: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_att TRUELAT1: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        nSTAT = nf90_get_att(ncid, NF90_GLOBAL, "TRUELAT2", Truelat2)
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_att TRUELAT2: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_att TRUELAT2: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        ...
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: inq_varid XLONG: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: inq_varid XLONG: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        nSTAT = nf90_inq_varid(ncid,"XLAT",lat_var_id)
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: inq_varid XLAT: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: inq_varid XLAT: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        ...
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_var XLONG: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_var XLONG: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
        ...
        nSTAT = nf90_get_var(ncid,lat_var_id,dum3d_sp, &
               start = (/1,1,1/),count = (/nx_fullmet,ny_fullmet,1/))
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_var XLAT: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_var XLAT: ',nf90_strerror(nSTAT)
          stop 1
        ENDIF
      ...
      ELSEif(Map_Proj.eq.3)THEN
        ! Mercator
        !  truelat1
        write(*,*)"WRF: MAP_PROJ=3 : Mercator : Not implemented"
        stop 1
      ELSEif(Map_Proj.eq.6)THEN
        ...
        !   pole_lon
        !   stand_lon
        write(*,*)"WRF: MAP_PROJ=6 : Lon-Lat : Not implemented"
        stop 1
      ELSE
        write(*,*)&
         "The MAP_PROJ global attribute is either not present or is"
        write(*,*)"not a recognized projection."
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_varid PB: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_varid PB: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: get_var PB: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: get_var PB: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF
         p_fullmet_sp(:) = dum4d_sp(1,1,:,1)
      nSTAT = nf90_inq_varid(ncid,"P",Ppert_var_id)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_varid P: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_varid P: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF
      nSTAT = nf90_get_var(ncid,Ppert_var_id,dum4d_sp, &
             start = (/1,1,1,1/),count = (/nx_fullmet,ny_fullmet,neta_fullmet,1/))
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: get_var P: ',nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: get_var P: ',nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      p_fullmet_RH_sp = p_fullmet_RH_sp * 100.0_sp   ! convert from hPa to Pa
       ...
       IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: close WRF file: ',nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: close WRF file: ',nf90_strerror(nSTAT)
          stop 1
       ENDIF
...
!##############################################################################
      ...
      IF(MR_iwindformat.eq.25.or.MR_iwindformat.eq.27)THEN
        ...
        IF(MR_runAsForecast)THEN
          write(*,*)"MR ERROR: iwf=25 and 27 cannot be used for forecast runs."
 ...
 110    format(a50,a1,i4,a1)
      ...
      ELSEIF(MR_iwindformat.eq.31)THEN
        ...
        DO iw = 1,MR_iwindfiles
          ...
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: nf90_open to read header:', &
                           nf90_strerror(nSTAT)
            write(6,*)'Could not open ',trim(ADJUSTL(MR_windfiles(iw)))
            write(6,*)'Exiting'
            stop 1
          ENDIF
          IF(iw.eq.1)THEN
            ...
            IF(nSTAT.ne.0)THEN
              write(6,*)'MR ERROR: inq_dimid time: ',nf90_strerror(nSTAT)
              write(9,*)'MR ERROR: inq_dimid time: ',nf90_strerror(nSTAT)
              write(6,*)"    Could not find dimension: ",Met_dim_names(1)
              stop 1
            ENDIF
          ...
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: inq_varid:',"time",nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: inq_varid:',"time",nf90_strerror(nSTAT)
            stop 1
          ENDIF
          ! time is an interger*4
          nSTAT = nf90_get_var(ncid,time_var_id,filetime_in_sp)
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: get_var:',"time",nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: get_var:',"time",nf90_strerror(nSTAT)
            stop 1
          ENDIF
          ...
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: inq_varid:',"reftime",nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: inq_varid:',"reftime",nf90_strerror(nSTAT)
            stop 1
          ENDIF

          nSTAT = nf90_get_var(ncid,reftime_var_id,tstring)
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: get_var:',"reftime",nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: get_var:',"reftime",nf90_strerror(nSTAT)
            stop 1
          ENDIF
 ...
 131      format(i4,1x,i2,1x,i2,1x,i2,1x,i2)
          ...
          nSTAT = nf90_close(ncid)
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: Could not close file',nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: Could not close file:',nf90_strerror(nSTAT)
            stop 1
          ENDIF
      ...
      ELSEIF(MR_iwindformat.eq.50)THEN
        ...
        DO iw = 1,MR_iwindfiles
          ...
          IF(iw.eq.1)THEN
            ...
            IF(nSTAT.ne.0)THEN
              write(6,*)'MR ERROR: inq_varid:',"Times",nf90_strerror(nSTAT)
              write(9,*)'MR ERROR: inq_varid:',"Times",nf90_strerror(nSTAT)
              stop 1
            ENDIF
            nSTAT = nf90_inquire_variable(ncid, time_var_id, invar, &
                xtype = var_xtype)
            IF(nSTAT.ne.0)THEN
              write(6,*)'MR ERROR: inq_variable:',"Times",nf90_strerror(nSTAT)
              write(9,*)'MR ERROR: inq_variable:',"Times",nf90_strerror(nSTAT)
              stop 1
            ENDIF
            allocate(MR_windfile_stephour(MR_iwindfiles,nt_fullmet))
            allocate(filetime_in_sp(nt_fullmet))
            If(nt_fullmet.gt.1)THEN
              write(*,*)"MR ERROR: Currently WRF files are expected to only have one"
              write(*,*)"       timestep/file"
              stop 1
            ENDIF
          ...
          nSTAT = nf90_get_var(ncid,time_var_id,Timestr_WRF,&
                         ...
                         count = (/19,1/))
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: get_var:',"Times",nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: get_var:',"Times",nf90_strerror(nSTAT)
            stop 1
          ENDIF
      ...
      ELSE
        ...
        DO iw = 1,MR_iwindfiles
          ...
          IF(nSTAT.eq.0)THEN
            ...
            IF(nSTAT.ne.0) THEN
              write(6,*)'MR ERROR: get_att:',"time",nf90_strerror(nSTAT)
              write(9,*)'MR ERROR: get_att:',"time",nf90_strerror(nSTAT)
              stop 1
            ENDIF
          ELSE
          ...
          ELSE
            ...
            nSTAT = nf90_inq_varid(ncid,'reftime',reftime_var_id)
            IF(nSTAT.ne.0)THEN
              write(*,*)"MR ERROR:  Could not read time:units or reftime"
              write(*,*)"        Windfile start time is not defined."
              stop 1
            ENDIF
            nSTAT = nf90_get_var(ncid,reftime_var_id,tstring2)
            IF(nSTAT.ne.0)THEN
              write(*,*)"MR ERROR:  Could not read reftime"
              write(*,*)"        Windfile start time is not defined."
              stop 1
            ENDIF
            DO i=1,30
              IF(tstring2(i:i+1).eq.'20'.or.tstring2(i:i+1).eq.'19')THEN
                write(*,*)"Found reference time: ",tstring2(i:31)
...
2100      FORMAT(20x,a11,i4,1x,i2,1x,i2,1x,i2,1x,i2,1x,i2)
 103      format(i4,1x,i2,1x,i2,1x,i2,1x,i2,1x,i2)
          ...
          IF(nSTAT.ne.0)THEN
            write(6,*)'MR ERROR: inq_variable: ',invar,nf90_strerror(nSTAT)
            write(9,*)'MR ERROR: inq_variable: ',invar,nf90_strerror(nSTAT)
            stop 1
          ENDIF
          IF(var_xtype.eq.NF90_FLOAT)THEN
            ...
            IF(nSTAT.ne.0)THEN
              write(6,*)'MR ERROR: get_var ',Met_dim_names(1),nf90_strerror(nSTAT)
              write(9,*)'MR ERROR: get_var ',Met_dim_names(1),nf90_strerror(nSTAT)
              stop 1
            ENDIF
          ...
          ELSE
            write(*,*)"MR ERROR: Unexpected time variable type ",Met_dim_names(i)
            stop 1
          ENDIF
...
!##############################################################################
      ...
      write(*,*)"--------------------------------------------------------------------------------"
      write(*,*)"----------                MR_Set_Met_Dims_Template_netcdf             ----------"
      write(*,*)"--------------------------------------------------------------------------------"
      ...
      iw = 1
      write(*,*)"Opening ",iw,trim(ADJUSTL(MR_windfiles(iw)))
      nSTAT = nf90_open(trim(ADJUSTL(MR_windfiles(iw))),NF90_NOWRITE,ncid)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: nf90_open to read header:', nf90_strerror(nSTAT)
        write(6,*)'Could not open ',trim(ADJUSTL(MR_windfiles(iw)))
        write(6,*)'Exiting'
        stop 1
      ELSE
        write(*,*)"Opened ",trim(ADJUSTL(MR_windfiles(iw)))
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_dimid ',Met_dim_names(i),nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_dimid ',Met_dim_names(i),nf90_strerror(nSTAT)
        stop 1
      ENDIF
      nSTAT = nf90_Inquire_Dimension(ncid,t_dim_id,name=name_dum,len=nt_fullmet)
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: Inquire_Dimension ',Met_dim_names(i),nf90_strerror(nSTAT)
        write(6,*)'MR ERROR: Inquire_Dimension ',Met_dim_names(i),nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      i = 4
      IF(.not.Met_dim_IsAvailable(i))THEN
        write(*,*)"MR ERROR: X/LON dimension is required and not listed"
        write(*,*)"          in template windfile specification file."
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_varid ',Met_dim_names(i),nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_varid ',Met_dim_names(i),nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      IF(var_xtype.eq.NF90_FLOAT)THEN
        ...
        IF(nSTAT.ne.0)THEN
          write(6,*)'MR ERROR: get_var ',Met_dim_names(i),nf90_strerror(nSTAT)
          write(9,*)'MR ERROR: get_var ',Met_dim_names(i),nf90_strerror(nSTAT)
          stop 1
        ENDIF
        ! copy to local variable
      ...
      ELSEIF(var_xtype.eq.NF90_DOUBLE)THEN
        ...
        deallocate(dum1d_dp)
      ELSE
        write(*,*)"MR ERROR: Unexpected dim variable type ",Met_dim_names(i)
        stop 1
      ENDIF
      ...
      i = 3
      IF(.not.Met_dim_IsAvailable(i))THEN
        write(*,*)"MR ERROR: Y/LAT dimension is required and not listed"
        write(*,*)"       in template windfile specification file."
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: Inquire_Dimension ',Met_dim_names(i),nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: Inquire_Dimension ',Met_dim_names(i),nf90_strerror(nSTAT)
        stop 1
      ENDIF
      nSTAT = nf90_inq_varid(ncid,Met_dim_names(i),var_id)
      IF(nSTAT.ne.0)THEN
      ...
      i = 2
      IF(.not.Met_dim_IsAvailable(i))THEN
        write(*,*)"MR ERROR: Pressure dimension is required and not listed"
        write(*,*)"       in template windfile specification file."
        stop 1
      ENDIF
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR: inq_varid: ',invar,nf90_strerror(nSTAT)
        write(9,*)'MR ERROR: inq_varid: ',invar,nf90_strerror(nSTAT)
        stop 1
      ENDIF
      ...
      IF(TimeHasFillVAttr)THEN
        ...
        write(*,*)"    Found fill value",fill_value_sp(MR_iwindformat)
      ELSE
        fill_value_sp(MR_iwindformat) = -9999.0_sp
      ENDIF
...
!##############################################################################
 ...
 131      format(a50,a15,i4,a19)
      ...
      IF(nSTAT.ne.0)THEN
        write(6,*)'MR ERROR open file:',infile,nf90_strerror(nSTAT)
        write(9,*)'MR ERROR open file:',infile,nf90_strerror(nSTAT)
        stop 1
      ENDIF
        ...
        do i=1,ict        !read subgrid at current time step
          if(MR_iwindformat.eq.25)THEN
            ...
            IF(var_xtype.eq.NF90_FLOAT)THEN
              ...
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: ',nf90_strerror(nSTAT),iicount(i),ny_submet,np_met_loc
                write(9,*)'MR ERROR: get_var: ',nf90_strerror(nSTAT)
                stop 1
              ENDIF
            ELSEIF(var_xtype.eq.NF90_SHORT)THEN
              nSTAT = nf90_get_var(ncid,in_var_id,temp3d_short(ileft(i):iright(i),:,:,:), &
                       ...
                       count = (/iicount(i),ny_submet,np_met_loc,1/))
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: ',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: ',nf90_strerror(nSTAT)
                stop 1
              ENDIF
            ENDIF
          elseif(MR_iwindformat.eq.50)then
            ...
            IF(ivar.eq.1)THEN
              ...
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: PHB',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: PHB',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              write(*,*)istep,"Reading ","PH"," from file : ",trim(adjustl(infile))
              nSTAT = nf90_inq_varid(ncid,"PH",in_var_id2)
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: inq_var: PH',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: inq_var: PH',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              nSTAT = nf90_get_var(ncid,in_var_id,dum3d_metP_aux(ileft(i):iright(i),:,:,:), &
                       ...
                       count = (/iicount(i),ny_submet,np_met_loc+1,1/))
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: PH',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: PH',nf90_strerror(nSTAT)
                stop 1
              ENDIF
            ...
            ELSEIF(ivar.eq.2)THEN
              ...
              IF(nSTAT.ne.0)THEN 
                write(6,*)'MR ERROR: get_var: U',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: U',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              DO ii=1,iicount(i)
            ...
            ELSEIF(ivar.eq.3)THEN
              ...
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: V',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: V',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              DO jj=1,ny_submet
            ...
            ELSEIF(ivar.eq.4)THEN
              ...
              nSTAT = nf90_get_var(ncid,in_var_id,temp3d_sp(ileft(i):iright(i),:,:,:), &
                       ...
                       count = (/iicount(i),ny_submet,np_met_loc+1,1/))
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: W',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: W',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              DO kk=1,np_met_loc
                MR_dum3d_metP(:,:,kk) = 0.5_sp*(temp3d_sp(:,:,kk  ,1) + &
            ...
            ELSEIF(ivar.eq.5)THEN
              ...
              write(*,*)istep,"Reading ","PB"," from file : ",trim(adjustl(infile))
              nSTAT = nf90_inq_varid(ncid,"PB",in_var_id1)
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: inq_var: PB',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: inq_var: PB',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              ...
              write(*,*)istep,"Reading ","P"," from file : ",trim(adjustl(infile))
              nSTAT = nf90_inq_varid(ncid,"P",in_var_id2)
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: inq_var: P',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: inq_var: P',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              nSTAT = nf90_get_var(ncid,in_var_id,dum3d_metP_aux(ileft(i):iright(i),:,:,:), &
                       start = (/iistart(i),jstart,1,iwstep/),       &
              ...
              nSTAT = nf90_get_var(ncid,in_var_id,temp3d_sp(ileft(i):iright(i),:,:,:), &
                       ...
                       count = (/iicount(i),ny_submet,np_met_loc,1/))
              IF(nSTAT.ne.0)THEN
                write(6,*)'MR ERROR: get_var: T',nf90_strerror(nSTAT)
                write(9,*)'MR ERROR: get_var: T',nf90_strerror(nSTAT)
                stop 1
              ENDIF
              DO ii=1,iicount(i)
                DO jj=1,ny_submet
            ...
            IF(nSTAT.ne.0)THEN
              ...
              write(9,*)'MR ERROR: get_var: ',nf90_strerror(nSTAT)
              write(*,*)i
              write(*,*)ileft(i),iright(i)
              write(*,*)iistart(i),jstart,1,iwstep
              write(*,*)iicount(i),ny_submet,np_met_loc,1
              stop 1
            ENDIF
          endif
          IF(nSTAT.ne.0) &
        ...
        If(MR_iwindformat.ne.50)THEN
          ...
          if(z_inverted)THEN ! reverse the vertical coordinate
            ...
            do i=1,np_met_loc
              ...
              !If(ivar.eq.4)write(*,*)MR_dum3d_metP(1,1,i),i,np_met_loc,np_fullmet
            enddo
          endif
            ...
            if(MR_iwindformat.eq.25)THEN
              ...
              IF(NC_version.eq.4)THEN
                ...
                IF(nSTAT.ne.0)THEN
                   ...
                   stop 1
                ENDIF
                write(*,*)"Need to write interp_iwf25_grid for float"
                stop 1
