...
\item \texttt{idf} :: Data file format code (ascii, netcdf, grib, etc)
 ...
 \item   4 : GRIB Edition 1 (Not implemented; used by NARR and NOAA-CIRES)
 \end{itemize}
...
implementation of the grib2 reader will read each of the required grib records into
...
is the variable ID according to Table \ref{VarIDs}.  Next is the scaling factor to convert
...
stops returning a non-zero stop-code (1).
Usage:\\
...
probing a single point and time, this utility calculates trajectories withing
either a GFS forecast package or the NCEP reanalysis files.
This utility can be compiled with either a \texttt{-DFORWARD} or \texttt{-DBACKWARD}
...
\texttt{get\_gfs0.5deg.sh} and \texttt{convert\_gfs0.5deg.sh}) is used for downloading and
...
Included in \texttt{autorun\_scripts} is a simple stand-alone script (\texttt{grib2nc.sh})
...
and do not template files.  These are listed in the table below
...
\hline
 20 & GFS & 4 & 0.5-degree \\
 21 & GFS & 3 & 1.0-degree \\
 22 & GFS & 193 & 0.25-degree \\
 23 & NCEP-DOE Reanalysis 2 & 2  & 2.5-degree \\
 24 & NASA-MERRA-2 Reanalysis & None & $0.625 \, \times \,0.5^{\circ}$ \\
 25 & NCEP/NCAR Reanalysis 1 & 2 & 2.5-degree \\
...
Outlines of North America Mesoscale (NAM) and Global Forecase System (GFS)
...
GFS 3,4,193, (dots correspond to the 1.0 degree NAM 3 grid)
}}
\end{center}\end{figure}
\clearpage
...
GFS NCEP Grid 3 (1-degree) :: \verb|gfs.t00z.pgrb2.1p00.f000.nc|\\
\verb|n003_template.txt| \\
\tiny \verbatiminput{n003_template.txt} \normalsize

GFS NCEP grid 4 (0.5-degrees) :: \verb|n004_template.txt| \\
\tiny \verbatiminput{n004_template.txt} \normalsize

GFS NCEP grid 193 (0.25-degrees) :: \verb|n193_template.txt| \\
...
NASA GEOS-5 Cp (0.625/0.5-degrees) :: \\
...
NASA GEOS-5 Np (0.25/0.3125-degrees) :: \\
